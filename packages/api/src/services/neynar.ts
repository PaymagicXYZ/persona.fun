...
import {
  CreatePostParams,
  type DeleteParams,
  type GetBulkCastsResponse,
  type GetBulkUsersResponse,
  type GetCastResponse,
  type GetCastsResponse,
  type GetChannelResponse,
  type GetUserResponse,
  type PostCastResponse,
  type QuoteCastParams,
  SubmitHashParams,
...
interface WebhookMethods {
  createWebhook(url: string, fids: number[]): Promise<WebhookResponse>
  getWebhook(webhookId: string): Promise<WebhookResponse>
  updateWebhook(webhookId: string, fids: number[], url: string): Promise<WebhookResponse>
  deleteWebhook(webhookId: string): Promise<void>
}

class NeynarService implements WebhookMethods {
  ...
  private async makeRequest<T>(
    ...
    const { maxRetries = 1, retryDelay = 10000, method = 'GET', body } = options ?? {}
  ...
  async createWebhook(url: string, fids: number[]): Promise<WebhookResponse> {
    return this.makeRequest<WebhookResponse>('/farcaster/webhook', {
      method: 'POST',
      body: JSON.stringify({
        name: 'persona_reply_agent',
        url,
        subscription: {
          'cast.created': {
            mentioned_fids: fids,
  ...
  async getWebhook(webhookId: string): Promise<WebhookResponse> {
    return this.makeRequest<WebhookResponse>(
      `/farcaster/webhook?webhook_id=${webhookId}`,
      {
        method: 'GET',
      }
    )
  ...
  async deleteWebhook(webhookId: string): Promise<void> {
    await this.makeRequest(`/farcaster/webhook/${webhookId}`, {
      method: 'DELETE',
    })
  }
}
